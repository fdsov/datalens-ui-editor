...
FROM ubuntu:${UBUNTU_VERSION} AS native-build-stage
...
RUN apt-get update && apt-get -y install nodejs g++ make python3
...
COPY ./ce-bundle /opt/app/ce-bundle
COPY app-builder.config.ts tsconfig.json /opt/app/

RUN npm run build && chown app /opt/app/dist/run

# Chart Editor bundled libraries (libs/dataset/v2 etc.) loaded into the isolated-vm sandbox
RUN npm run build:ce-bundle
...
COPY --from=native-build-stage /opt/app/ce-dist /opt/app/ce-dist
