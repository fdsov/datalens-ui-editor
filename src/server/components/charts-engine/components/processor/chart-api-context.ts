...
export const getChartApiContext = (args: GetChartApiContextArgs): ChartApiContext => {
    ...
    // Editor tab names (Urls/JavaScript/UI) are aliases of the wizard tab names
    // (Sources/Prepare/Controls); both are handled so the shared context fits either path.
    if (name === 'Sources' || name === 'Urls') {
        api.getSortParams = () => getSortParams(params);
    }

    if (name === 'Sources' || name === 'Prepare' || name === 'Urls' || name === 'JavaScript') {
    ...
    if (
        ...
        if (name === 'Prepare' || name === 'JavaScript') {
