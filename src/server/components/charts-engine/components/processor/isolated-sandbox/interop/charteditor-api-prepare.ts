...
// Injected into the isolate by isolated-sandbox-chart-builder from config.data.meta.links —
// maps editor link aliases (e.g. 'PlanDataset') to real entry ids. Backs ChartEditor.getId().
declare const __links: Record<string, string> | undefined;
...
// @ts-ignore
// `Editor`/`editor` are legacy global aliases for `ChartEditor` kept for backward
// compatibility with older editor charts (e.g. authored in managed DataLens).
this.ChartEditor = this.chartEditor = this.Editor = this.editor = chartEditor;
