...
export const getIsolatedSandboxChartBuilder = async (
    ...
    // Editor link aliases (config.data.meta.links) → real entry ids, backs ChartEditor.getId().
    let links: Record<string, string> = {};
    try {
        const parsedMeta = config.data.meta ? JSON.parse(config.data.meta) : null;
        if (parsedMeta && typeof parsedMeta.links === 'object' && parsedMeta.links) {
            links = parsedMeta.links;
        }
    } catch {
        links = {};
    }
    ...
    context.evalSync(
        `
         ...
         let __links = JSON.parse('${JSON.stringify(links)}');
    `,
    );
    ...
    return {
        ...
        buildModules: async ({subrequestHeaders, ctx, onModuleBuild}) => {
