...
export const prepareImportChartData = async (
    ...
    const editorData = chartOptions.data as unknown as Record<string, any>;
    const isEditorChart =
        chartOptions.type === 'table_node' ||
        Boolean(editorData && !('shared' in editorData) && 'config' in editorData);

    if (isEditorChart) {
        const data: Record<string, any> = {...editorData};
        let missedMapping = false;

        let metaObj: Record<string, any> = {};
        try {
            metaObj =
                typeof editorData.meta === 'string'
                    ? JSON.parse(editorData.meta)
                    : {...(editorData.meta || {})};
        } catch (e) {
            metaObj = {};
        }

        const sourceLinks: Record<string, string> = metaObj.links || {};
        const mappedLinks: Record<string, string> = {};
        const entryLinks: Record<string, string> = {};

        Object.entries(sourceLinks).forEach(([alias, oldId]) => {
            const mapped = idMapping[oldId as string];
            if (mapped) {
                mappedLinks[alias] = mapped;
                entryLinks[alias] = mapped;
            } else {
                missedMapping = true;
                mappedLinks[alias] = TRANSFER_UNKNOWN_ENTRY_ID;
            }
        });

        metaObj.links = mappedLinks;
        data.meta =
            typeof editorData.meta === 'string' ? JSON.stringify(metaObj, null, 4) : metaObj;
