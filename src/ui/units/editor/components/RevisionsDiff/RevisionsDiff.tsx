...
import {Dialog, Switch, Tab, TabList, TabProvider} from '@gravity-ui/uikit';
...
export const RevisionsDiff = (props: RevisionsDiffProps) => {
    ...
    return (
        <div className={b('content')}>
            ...
            <TabProvider value={currentTabId} onUpdate={setCurrentTabId}>
                <TabList className={b('pane-tabs')}>
                    {props.tabsData.map(({id, name}) => (
                        <Tab key={id} value={id}>
                            {name}
                        </Tab>
                    ))}
